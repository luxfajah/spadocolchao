...
import { RegisterCustomerModal } from "../../_components/RegisterCustomerModal"
...
export function VisitasClient({ sellerId }: { sellerId: string | null }) {
  ...
  // Prefill data for RegisterCustomerModal
      ...
      if (res.ok) {
        ...
        if (status === "COMPLETED") {
          if (selectedVisit.customerId) {
            ...
            let prefilledAddr: any = {}
            if (selectedVisit.clientAddress) {
              const parts = selectedVisit.clientAddress.split(', ')
              prefilledAddr.street = parts[0] || ""
              const numMatch = parts[1]?.match(/nº\s*(.*)/)
              prefilledAddr.number = numMatch ? numMatch[1] : parts[1] || ""
              prefilledAddr.neighborhood = parts[2] || ""
              prefilledAddr.city = parts[3] || ""
              prefilledAddr.state = parts[4] || ""
            }

            setRegisterInitialData({
              fullName: selectedVisit.clientName || "",
              phone: selectedVisit.clientPhone || "",
              ...prefilledAddr
            })
  ...
  return (
    <>
      ...
      <RegisterCustomerModal
        isOpen={modalMode === "register_client"}
        onClose={() => { setModalMode(null); setRegisterInitialData(null) }}
        sellerId={sellerId}
