...
export function NewBoxForm({ product, onAdd, onCancel }: { product: any, onAdd: (details: any, price: number) => void, onCancel: () => void }) {
  ...
  const priceBreakdown = useMemo(() => {
    let base = Number(product.price) || 650;
    let extras = 0;
    
    if (data.optStructureReinforce) extras += 100;
    if (data.optHardwareReplacement) extras += 150;
  ...
  const steps = [
    ...
    { id: 2, title: "Opcionais", icon: Hammer },
        ...
        {step === 1 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="space-y-4">
              <label className="text-sm font-bold text-slate-700 block">Tipo de Box</label>
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-slate-700 block">Tamanho Comercial</label>
        ...
        {step === 2 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2 uppercase tracking-tight">
              <Settings2 size={24} className="text-brand-900" /> Opcionais de Fabricação
            </h3>
            ...
            <div className="grid grid-cols-1 gap-4">
              <label className={cn(
                ...
                <div>
                  <span className="text-sm font-black text-slate-900 block uppercase tracking-tight">Estrutura Reforçada</span>
                  <span className="text-xs text-slate-600">Madeiramento de maior densidade para suporte de peso extra.</span>
                </div>
              </label>
              ...
              <label className={cn(
                ...
                <div>
                  <span className="text-sm font-black text-slate-900 block uppercase tracking-tight">Amortecedores Premium</span>
                  <span className="text-xs text-slate-600">Pistões de alta performance para abertura suave (Baú).</span>
                </div>
              </label>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <select name="topFabricId" value={data.topFabricId} onChange={handleChange} className="flex h-12 w-full rounded-xl border border-input bg-background px-3 text-sm focus:ring-2 focus:ring-brand-500">
                  ...
                  {topFabrics.map((f: any) => <option key={f.id} value={f.id}>{f.name}</option>)}
                </select>
              </div>
              <div className="space-y-2">
                ...
                <select name="sideFabricId" value={data.sideFabricId} onChange={handleChange} className="flex h-12 w-full rounded-xl border border-input bg-background px-3 text-sm focus:ring-2 focus:ring-brand-500">
                  ...
                  {sideFabrics.map((f: any) => <option key={f.id} value={f.id}>{f.name}</option>)}
                </select>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-muted-foreground uppercase">Jogo de Pés</label>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="p-6 bg-slate-50 rounded-3xl border shadow-sm">
              <h3 className="text-lg font-black mb-6 uppercase tracking-tight flex items-center gap-2">
                <Check className="text-green-500" /> Resumo do Box Novo
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-white rounded-xl border space-y-2">
                    <span className="text-[10px] font-black text-brand-900 uppercase tracking-widest">Especificações</span>
                    <ul className="text-sm space-y-1 font-medium">
                      {data.optStructureReinforce && <li>• Estrutura Reforçada Inclusa</li>}
                      {data.optHardwareReplacement && <li>• Ferragens Premium</li>}
                    </ul>
                  </div>
                </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <label className="text-sm font-bold uppercase tracking-widest text-slate-500">Nível de Qualidade</label>
              ...
              <div className="bg-brand-950 text-white p-8 rounded-[40px] shadow-2xl flex flex-col justify-between">
                <div className="space-y-6">
                  <h3 className="text-xl font-black uppercase tracking-tighter text-slate-400">Total do Produto</h3>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-400 font-bold uppercase tracking-widest">Preço Base</span>
