...
async function main() {
  const employeeId = "cmoufsi540001cepsmcfq0zzv"; // Edcleisson
  ...
  console.log(`Encontrados ${absentDays.length} dias de falta para Edcleisson em Abril.`);
  ...
  for (const day of absentDays) {
    ...
    // Escala Produção Integral: 08-12, 14-18
    ...
    console.log(`  - Preenchendo dia ${dateKey} com escala normal...`);
    await prisma.attendanceDay.update({
        ...
        data: {
            ...
            adjustmentReason: "Ajuste de falta retroativo conforme orientação: Trabalhou normalmente mas não houve registro no relógio."
        }
    });
  }

  // Recalcular o sumário do espelho para refletir os ajustes nos totais
